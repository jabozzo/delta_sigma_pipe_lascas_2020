...
TESTS = (0, 1, 2,)
S_TAU = 0.5
PLOT_STD = not TOGETHER  # Set to true to plot standard deviation of aggregation.
...
# My excuses for some of the code that follows, it was cobbled together in a
# hurry
...
def plot2d(name, dat, name_axes, x_axes=None, y_axes=None, noise=False, hide_constants=False, ax=None, color=None, first_black=False, skip_first=False, legend_handlers=None, index=0, grading=False):
    if color is None:
        color = (0.0, 0.0, 0.0,)
    ...
    color_per = (0, 0, 0,) if first_black else (90/255, 70/255, 255/255,)
    color_nav = (0, 0, 0,) if first_black else (50/255, 135/255, 255/255,)

    def pplot(ax, mean, std, idx, color, label, apply_off=False, linestyle=None, zorder=0):
        if apply_off:
            styles = (None, '--', '-.', ':',)
            linestyle = styles[idx[1] % len(styles)]

        l_color = color + (1 - (idx[1] / 5),) if grading else color
        l_color = l_color + (1,)*(4 - len(l_color))
        lx_axes_none = x_axes is None
        lx_axes = np.arange(np.size(mean[idx])) if lx_axes_none else x_axes


        h = ax.plot(lx_axes, mean[idx], color=l_color, label=label, linestyle=linestyle, zorder=zorder+3)

        if PLOT_STD:
            ll_color = l_color[:3] + (l_color[3] * 0.2,)

            high = mean[idx] + std[idx]
            low = mean[idx] - std[idx]

            ll_color = l_color[:3] + (l_color[3] * 0.4,)
            ax.plot(lx_axes, high, color=ll_color, zorder=zorder+2, linestyle=linestyle)
            ax.plot(lx_axes, low, color=ll_color, zorder=zorder+2, linestyle=linestyle)
    ...
    for ii in reversed(range(n)):
        ...
        if ii == 0 and first_black is True:
            ...
            l_color = (0, 0, 0,)
            zorder = 10
        h = pplot(ax, cal_snr_m, cal_snr_s, idx, l_color, legend, apply_off=True, zorder=zorder)
        legend_handlers.append(h)

        if ii == 0:
            if not hide_constants:
                h0 = pplot(ax, nav_snr_m, nav_snr_s, idx, color_nav, "No calibration", linestyle='--', zorder=zorder)
                h1 = pplot(ax, per_snr_m, per_snr_s, idx, color_per, "Perfect knowledge", linestyle='-.', zorder=zorder)
    ...
    for name in names:
        with open(name, 'rb') as f:
            ...
            if TOGETHER:
                ...
                if 2 in TESTS:
                    f, ax, h2 = plot2d(*t_args, **t_kwargs, ax=ax, color=(0,0,brightness), skip_first=True, index=2, legend_handlers=h2)
                    h2 = reversed(h2)

                if 1 in TESTS:
                    f, ax, h1 = plot2d(*r_args, **r_kwargs, ax=ax, color=(0, brightness,0), skip_first=True, index=1, legend_handlers=h1)
                    h1 = reversed(h1)
                ...
                single_params = {"hide_constants":False, "grading":True}
                if 0 in TESTS:
                    plot2d(*u_args, **u_kwargs, color=(0.8,0,1), **single_params)

                if 1 in TESTS:
                    plot2d(*r_args, **r_kwargs, color=(0.8,0,1), **single_params)

                if 2 in TESTS:
                    plot2d(*t_args, **t_kwargs, color=(0.8,0,1), **single_params)
